...
export class ProfessionalPDFService {
  ...
  transformCalculatorData(calcData) {
    const { customer, generators, services, calculation, metadata } = calcData;
  ...
  buildServiceRows(services, calculation) {
    ...
    services.forEach(serviceType => {
      const serviceData = calculation.breakdownByService?.[serviceType];
      if (serviceData) {
        // Calculate quarterly pricing based on frequency
        const annualCost = serviceData.grandTotal || 0;
    ...
    if (calculation.mileageCost && calculation.mileageCost > 0) {
